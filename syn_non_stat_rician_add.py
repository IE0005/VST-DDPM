...
def main():
    ...
    p.add_argument("--n_aug", type=int, default=5,
                   help="How many noisy realizations per input image.")
    ...
    with open(manifest_path, "w", newline="") as f:
        ...
        for i, src_path in enumerate(paths):
            ...
            clean_npy_path = ""
            if args.save_clean_npy:
                clean_npy_path = str(clean_dir / (Path(src_path).stem + "_clean.npy"))
            ...
            for a in range(args.n_aug):
                ...
                sid = f"{Path(src_path).stem}_aug{a:03d}"
                noisy_npy = noisy_dir / f"{sid}_noisy.npy"
                sigma_npy = sigma_dir / f"{sid}_sigma.npy"
                ...
                sigma_png = ""
                if args.save_png:
                    noisy_png = str(noisy_png_dir / f"{sid}_noisy.png")
                    sigma_png = str(sigma_png_dir / f"{sid}_sigma.png")
