# mri_denoiser.py
# Pure denoiser training & inference (DDPM-style backbone, x0-pred objective)
# Works with single-channel MRI (channels=1) by default.
...
def cast_tuple(t, length=1): return t if isinstance(t, tuple) else ((t,) * length)
def divisible_by(numer, denom): return (numer % denom) == 0
...
class Unet(Module):
    ...
    @property
    def downsample_factor(self): return 2 ** (len(self.downs) - 1)
    ...
    def forward(self, x, time, x_self_cond=None):
        ...
        for block1, block2, attn, downsample in self.downs:
            ...
            x = block2(x, t); x = attn(x) + x; h.append(x)
            x = downsample(x)
...
# --------------------
# GaussianDiffusion (x0 prediction, denoiser mode)
# --------------------
...
class GaussianDiffusion(Module):
    def __init__(
        ...
        *,
        ...
        objective='pred_x0',          # <--- key: predict clean image
        ...
        super().__init__()
        assert model.channels == model.out_dim, "Model out_dim must match channels for x0 prediction"
        self.model = model
        ...
        if isinstance(image_size, int):
            if image_size > 0:
                ...
                image_size = None  # free-size mode
    ...
    @property
    def device(self): return self.betas.device
    ...
    # --- denoiser training loss (paired or synthetic) ---

    @torch.no_grad()
    def _rician_corrupt(self, x0, sigma):
        """
        Rician corruption for magnitude MRI: sqrt((x0 + n1)^2 + n2^2), with n1,n2 ~ N(0, sigma^2)
        x0 is assumed in [-1,1]; sigma is scalar or (B,1,1,1)
        """
        n1 = torch.randn_like(x0) * sigma
        n2 = torch.randn_like(x0) * sigma
        # map to positive magnitude-like space (simple sim): use |.| to mimic magnitude
        y = torch.sqrt((x0 + n1) ** 2 + (n2) ** 2)
        # bring back to roughly [-1,1] scale by centering (optional small correction)
        y = (y - y.mean(dim=(1,2,3), keepdim=True)) / (y.std(dim=(1,2,3), keepdim=True) + 1e-6)
        y = torch.clamp(y, -3, 3) / 3.0  # tame dynamic range
        return y

    def p_losses(self, x_start, t, noise=None, offset_noise_strength=None, x_noisy_override=None, rician=False, rician_sigma=0.1):
        b, c, h, w = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))
        offset_noise_strength = default(offset_noise_strength, self.offset_noise_strength)

        if x_noisy_override is None:
            # synthetic corruption
            if rician:
                # use the marginal variance at timestep t as sigma; or fixed rician_sigma
                sigma = torch.full_like(x_start, rician_sigma)
                x = self._rician_corrupt(x_start, sigma)
            else:
            ...
            # paired pathway: noisy already given
            x = x_noisy_override
    ...
    def forward(self, img, *args, **kwargs):
        """
        Accepts:
          - clean-only batch: img -> synthetic corruption
          - paired batch: (x_noisy, x_clean)
        """

        # -------- choose t range that matches sigma in [0.05, 0.125] --------
        sigma_min = 0.005
        sigma_max = 0.10
        t_min = _match_sigma_to_t(self.alphas_cumprod, sigma_min ** 2)
        t_max = _match_sigma_to_t(self.alphas_cumprod, sigma_max ** 2)
        # --------------------------------------------------------------------
        ...
        if isinstance(img, (tuple, list)) and len(img) == 2:
            ...
            # >>> use restricted t range here <<<
            t = torch.randint(t_min, t_max + 1, (b,), device=device).long()

            x_clean = self.normalize(x_clean)
            x_noisy = self.normalize(x_noisy)
            return self.p_losses(x_clean, t, x_noisy_override=x_noisy, *args, **kwargs)
        ...
        # >>> and also use same restricted t range here <<<
        t = torch.randint(t_min, t_max + 1, (b,), device=device).long()

        img = self.normalize(img)
        return self.p_losses(img, t, *args, **kwargs)

    # ---- q_sample for Gaussian synthetic corruption ----
    ...
    def one_shot_denoise(self, noisy_01, sigma_est=None):
        """
        Given a noisy image in [0,1], predict x0 directly (fast).
        If sigma_est is provided (std in [0,1] scale), match a timestep; else pick mid-t.
        """
        x_t = self.normalize(noisy_01.to(self.device))
        B = x_t.size(0)
        if sigma_est is None:
            t_idx = int(0.6 * (self.num_timesteps - 1))
        else:
            t_idx = _match_sigma_to_t(self.alphas_cumprod, float(sigma_est) ** 2)
        t = torch.full((B,), t_idx, device=self.device, dtype=torch.long)
        x0_hat = self.model_predictions(x_t, t, clip_x_start=True).pred_x_start
        return self.unnormalize(x0_hat)

    @torch.no_grad()
    def few_step_denoise(self, noisy_01, sigma_est=None, steps=20, eta=0.0):
        """
        Short DDIM walk from matched t -> 0 (higher quality than one-shot).
        """
        self.ddim_sampling_eta = eta
        x = self.normalize(noisy_01.to(self.device))
        B = x.size(0)
        if sigma_est is None:
            t_start = int(0.6 * (self.num_timesteps - 1))
        else:
            t_start = _match_sigma_to_t(self.alphas_cumprod, float(sigma_est) ** 2)

        times = torch.linspace(t_start, 0, steps + 1, dtype=torch.long, device=self.device)
...
class ImageFolderDataset(TorchDataset):
    """
    Clean MRI images only (for synthetic noise training).
    """
    def __init__(self, folder, image_size, exts=('png','jpg','jpeg','tiff'), convert_image_to='L', center_crop=True):
        super().__init__()
        self.paths = [p for ext in exts for p in Path(folder).glob(f'**/*.{ext}')]
        # self.transform = T.Compose([
        #     T.Lambda(partial(convert_image_to_fn, convert_image_to)),
        #     T.Resize(image_size),
        #     T.CenterCrop(image_size) if center_crop else nn.Identity(),
        #     T.ToTensor()
        # ])
        transform_list = [T.Lambda(partial(convert_image_to_fn, convert_image_to))]
...
class PairedMRIDataset(TorchDataset):
    """
    Loads (noisy, clean) pairs from two folders with mirrored structure or matched filenames.
    """
    def __init__(self, noisy_folder, clean_folder, image_size, exts=('png','jpg','jpeg','tiff'), convert_image_to='L'):
        super().__init__()
        self.noisy_paths = [p for ext in exts for p in Path(noisy_folder).glob(f'**/*.{ext}')]
        self.clean_root = Path(clean_folder)
        # self.transform = T.Compose([
        #     T.Lambda(partial(convert_image_to_fn, convert_image_to)),
        #     T.Resize(image_size),
        ...
        transform_list = [T.Lambda(partial(convert_image_to_fn, convert_image_to))]
        if image_size and image_size > 0:
            transform_list += [T.Resize(image_size), T.CenterCrop(image_size)]
        transform_list.append(T.ToTensor())
        self.transform = T.Compose(transform_list)

    def __len__(self): return len(self.noisy_paths)

    def _match_clean(self, noisy_path: Path):
        # try same relative structure, else search by filename
        try:
            rel = noisy_path.relative_to(noisy_path.parents[0])
            cand = self.clean_root / rel
            if cand.exists():
                return cand
        except Exception:
            pass
        matches = list(self.clean_root.rglob(noisy_path.name))
        if len(matches) == 0:
            raise FileNotFoundError(f"No clean match for {noisy_path.name}")
        return matches[0]

    def __getitem__(self, idx):
        noisy_path = self.noisy_paths[idx]
        clean_path = self._match_clean(noisy_path)
...
class Trainer:
    def __init__(
        ...
        *,
        ...
        self.rician = rician
        self.rician_sigma = rician_sigma
        ...
        if HAS_ACCELERATE:
            ...
            class _Dummy:
                def __init__(self): self.is_main_process=True
        ...
        dl = DataLoader(ds, batch_size=train_batch_size, shuffle=True, pin_memory=True, num_workers=min(8, cpu_count()))
        ...
        self.ema = EMA(self.model, beta=ema_decay, update_every=ema_update_every).to(self.device) if HAS_EMA and self.accelerator.is_main_process else None

        self.results_folder = Path(results_folder); self.results_folder.mkdir(exist_ok=True)
    ...
    def save(self, tag='latest'):
        if not getattr(self.accelerator, 'is_main_process', True): return
        data = {
            'step': self.step,
    ...
    def train(self):
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not getattr(self.accelerator, 'is_main_process', True)) as pbar:
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    if isinstance(batch, (tuple, list)) and len(batch) == 2:
                        ...
                        loss = self.model((noisy, clean), rician=False)
                    # else:
                    #     clean = batch.to(self.device)
                    #     # synthetic corruption path; toggle rician if wanted
                    #     loss = self.model(clean, rician=self.rician, rician_sigma=self.rician_sigma)
                    else:
                        clean = batch.to(self.device)
                ...
                if self.step % self.save_and_sample_every == 0 and getattr(self.accelerator, 'is_main_process', True):
                    # quick visual: take a clean batch, corrupt, then denoise few steps
                    self.model.eval()
                    with torch.no_grad():
                        # # sample visual
                        # if not isinstance(batch, (tuple, list)):
                        #     clean_vis, _ = pad_to_multiple(clean_vis, multiple=8)
                        #     vis_noisy, _ = pad_to_multiple(vis_noisy, multiple=8)
                        # else:
                        #     # corrupt one batch artificially to visualize
                        #     b = min(clean.shape[0], self.num_samples)
                        #     clean_vis = clean[:b].clamp(0,1)
                        #     t_vis = torch.randint(0, self.model.num_timesteps, (b,), device=self.device)
                        #     noisy_vis = self.model.q_sample(self.model.normalize(clean_vis), t_vis)
                        #     vis_noisy = self.model.unnormalize(noisy_vis).clamp(0,1)
                        #
                        # den = self.model
                        # denoised = den.few_step_denoise(vis_noisy, sigma_est=None, steps=20, eta=0.0)
                        # grid = torch.cat([vis_noisy, denoised], dim=0)
                        # utils.save_image(grid, str(self.results_folder / f'step-{self.step:06d}.png'), nrow=vis_noisy.shape[0])
                        # sample visual
                        if isinstance(batch, (tuple, list)) and len(batch) == 2:
                            # paired: we already have real noisy
                            b = min(noisy.shape[0], self.num_samples)
                            vis_noisy = noisy[:b].clamp(0, 1)
                        else:
                            # clean-only dataset: synthesize a noisy preview
                            b = min(clean.shape[0], self.num_samples)
                            clean_vis = clean[:b].clamp(0, 1)
                            t_vis = torch.randint(0, self.model.num_timesteps, (b,), device=self.device)
                            noisy_vis = self.model.q_sample(self.model.normalize(clean_vis), t_vis)
                        ...
                        # --- ensure multiples of 8 for the UNet ---
                        vis_noisy, _ = pad_to_multiple(vis_noisy, multiple=8)
                        # only pad clean_vis if it was created in the clean-only branch
                        if 'clean_vis' in locals():
                            clean_vis, _ = pad_to_multiple(clean_vis, multiple=8)

                        den = self.model
                        denoised = den.few_step_denoise(vis_noisy, sigma_est=None, steps=20, eta=0.0)
                        grid = torch.cat([vis_noisy, denoised], dim=0)
                        utils.save_image(grid, str(self.results_folder / f'step-{self.step:06d}.png'),
                                         nrow=vis_noisy.shape[0])
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="MRI Denoiser (DDPM x0-pred)")
    parser.add_argument("--mode", type=str, default="train", choices=["train", "denoise_one", "denoise_few"])
    parser.add_argument("--image_size", type=int, default=256)
    parser.add_argument("--train_clean", type=str, help="Folder of clean MRIs (for synthetic noise)")
    parser.add_argument("--train_noisy", type=str, help="Folder of noisy MRIs (paired mode)")
    parser.add_argument("--train_batch_size", type=int, default=8)
    parser.add_argument("--train_steps", type=int, default=10000)
    parser.add_argument("--save_dir", type=str, default="./results_mri_denoise")
    parser.add_argument("--timesteps", type=int, default=1000)
    parser.add_argument("--sampling_timesteps", type=int, default=50)
    parser.add_argument("--rician", action="store_true", help="Use simple rician corruption in synthetic mode")
    parser.add_argument("--rician_sigma", type=float, default=0.1)
...
#    parser.add_argument("--sigma_est", type=float, default=None, help="Estimated noise std in [0,1] for matching the timestep; " "e.g. 0.05, 0.10, 0.125. If None, use default mid-t.")
    parser.add_argument("--resume", action="store_true", help="Resume training from model-latest.pt in save_dir if it exists")
    parser.add_argument("--infer_h", type=int, default=181, help="Height that the model was trained on (for inference resize)")
    parser.add_argument("--infer_w", type=int, default=217, help="Width that the model was trained on (for inference resize)")
    ...
    if args.mode == "train":
        if args.train_noisy and args.train_clean:
            ...
            raise ValueError("Provide --train_clean (synthetic) or both --train_noisy and --train_clean (paired).")

        trainer = Trainer(
            diffusion_model=diff,
            folder=folder,
            train_batch_size=args.train_batch_size,
            train_lr=2e-4, #1e-4 was used previously 
            train_num_steps=args.train_steps,
            results_folder=args.save_dir,
            rician=args.rician,
            rician_sigma=args.rician_sigma
        )
        if args.resume:
            trainer.load("latest")
        ...
        trainer.train()
         ...
         # require weights + (either a single file or a folder)
        ...
        def _denoise_tensor(x_noisy):
            with torch.no_grad():
                if args.mode == "denoise_one":
                    # fast single-shot denoising
                    return diff.one_shot_denoise(x_noisy)
                else:  # mode == "denoise_few"
                    # short DDIM denoising, no sigma given
        ...
        def _load_to_tensor(path):
            # 1) load real MRI slice (any size)
            img = Image.open(path).convert("L")
            orig_w, orig_h = img.size  # (width, height)
        
            # 2) to tensor [1,1,H,W] in [0,1]
            x = to_tensor(img).unsqueeze(0).to(device)
        
            # 3) pad to multiple of 8 for the UNet
            x_padded, pad = pad_to_multiple(x, multiple=8)
        
            # 4) denoise on padded image
            ...
            return x_denoised, (orig_w, orig_h)
        ...
        if args.noisy_folder:
            ...
            for p in img_paths:
                x_denoised, orig_size = _load_to_tensor(p)
                out_path = os.path.join(args.out_folder, os.path.basename(p))
                utils.save_image(x_denoised, out_path, nrow=1)
                if orig_size:
                    print(f"Saved: {out_path}  (orig size {orig_size[0]}x{orig_size[1]})")
                else:
            ...
            x_denoised, orig_size = _load_to_tensor(args.noisy_image)
            utils.save_image(x_denoised, args.out_image, nrow=1)
            if orig_size:
                print(f"Saved denoised image to: {args.out_image}  (orig size {orig_size[0]}x{orig_size[1]})")
            else:
